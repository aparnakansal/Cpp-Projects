...
public:
	void shellSort();
	// Function to Shellsort in descending order
...
{
	// based on insertionSort (ascending order) on p.543 of Malik
	// To change it to descending order, we need to change two places:
	// (1) the if statement within the for loop: change from < to >
	// (2) the condition for do {...} while: change from > to <
   int firstOutOfOrder, location;
   elemType temp;

   for (firstOutOfOrder = begin + inc; firstOutOfOrder < length;
...
void arrayListType<elemType>::insert(const elemType& insertItem)
{
    ...
    else
    {
        ...
        if (loc == -1)    //the item to be inserted
			...
			cerr << "the item to be inserted ( " << insertItem << ") is already in "
