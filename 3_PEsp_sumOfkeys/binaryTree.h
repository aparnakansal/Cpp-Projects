...
{
public:
	elemType sumOfKeys(int nodeType) const;
	// Function to calculate sum of keys of nodes.
	// nodeType is one of the followings:
	//   ALL_LEVEL sum of keys of all nodes
	//   ODD_LEVEL sum of keys of all odd-level nodes
	//   EVEN_LEVEL sum of keys of all even-level nodes
	// The return value is the sum.  Each invocation is limited
	// to traverse the tree only once.
...
private:
	elemType recSumOfKeys(binaryTreeNode<elemType>* root, int level, int nodeType) const;
...
template <class elemType>
elemType binaryTreeType<elemType>::sumOfKeys(int nodeType) const
{
	assert(nodeType == ALL_LEVEL || nodeType == ODD_LEVEL || nodeType == EVEN_LEVEL);

	// start at root at level 0
	return recSumOfKeys(root, 0, nodeType);
} // end of sumOfKeys

template <class elemType>
elemType binaryTreeType<elemType>::recSumOfKeys(binaryTreeNode<elemType>* nodePtr, int level, int nodeType) const
{
	if (nodePtr == NULL)
		return 0;

	elemType v = 0;
	if ((nodeType == ALL_LEVEL) ||
		((nodeType == ODD_LEVEL) && (level % 2 == 1)) ||
		((nodeType == EVEN_LEVEL) && (level % 2 == 0))
		) {
		v = nodePtr->info;
	}

	return v + recSumOfKeys(nodePtr->llink, level + 1, nodeType) + recSumOfKeys(nodePtr->rlink, level + 1, nodeType);
