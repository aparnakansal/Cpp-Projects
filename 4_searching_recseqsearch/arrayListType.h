...
private:
    int recseqsearch(const elemType& item, int first, int last);
...
int arrayListType<elemType>::recursiveseqsearch(const elemType& item){
	int a=recseqsearch(item, 0, length-1);
	return a;
}

template <class elemType>
int arrayListType<elemType>::recseqsearch(const elemType& item, int first, int last)
{
	if(first<=last){
		if(list[first]==item){
			return first;
