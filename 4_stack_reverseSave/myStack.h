...
bool stackType<Type>::operator==(const stackType<Type>& otherStack) const{
	...
	else if(this->maxStackSize!=otherStack.maxStackSize || this-> stackTop!=otherStack.stackTop){
		return false;
	}
	else{
...
void stackType<Type>::reverseStack(stackType<Type> &otherStack){
	if (maxStackSize != otherStack.maxStackSize)
	   {
		   cout << "The two stacks are of different size" << endl;
		   cout << "Cannot copy stack in reverse order" << endl;
	   }
	   else
	   {
			otherStack.stackTop = stackTop;

			for (int j = 0; j < stackTop; j++)   // copy other stack into this stack
